...
from megatron.core.utils import deprecate_inference_params, make_viewless_tensor, log_single_rank

from vlm.models.vision.rice_vit_model import RiceViTModel
from vlm.models.rice_gpt.vision_projector import VisionProjector
from vlm.models.qwen_vl.utils import get_inputs_on_this_cp_rank


class RiceGPTModel(MegatronModule):
    """Rice-GPT multi-modal model.
    ...
    def __init__(
        ...
        language_position_embedding_type: Literal[
        ...
        log_single_rank(
            ...
            logging.WARNING,
            "RiceGPTModel is work in progress. Features are missing and methods can change.",
        )
        ...
        if self.add_encoder:
            vision_model_type = getattr(vision_transformer_config, "vision_model_type", "rice")
            ...
            self.vision_projection = VisionProjector(
                ...
                input_size=vision_projection_input_size,
                output_size=language_transformer_config.hidden_size
            )
        ...
        if self.add_decoder:
            language_model_type = getattr(language_transformer_config, "language_model_type", "gpt")
            assert "gpt" in language_model_type, "RiceGPTModel only supports GPT model"  
    ...
    def freeze(
        ...
        #         if "router" in n or "gate" in n or "expert" in n:
    ...
    def forward(
        ...
        *,
        ...
        if use_inference_kv_cache:
            ...
            image_embeddings = torch.tensor([], device=input_ids.device).reshape(       # TODO: 왜 여기에 걸리는거지?
                0, 0, 0
            )
        elif self.add_encoder:
            if images is not None:
                assert has_images, "images tensor is empty but expected to have images."
                ...
                image_embeddings = self.vision_projection(image_embeddings, window_index)

                n_image_tokens = (input_ids == self.config.image_token_id).sum().item()
                n_image_features = image_embeddings.shape[0]
                if has_image_tokens and n_image_tokens != n_image_features:
                    raise ValueError(
        ...
        if self.pre_process:
            ...
            if use_inference_kv_cache or (images is None and pixel_values_videos is None):
                ...
                if has_images and has_image_tokens:
                ...
                elif has_images and not has_image_tokens and image_embeddings is not None:
                    # dummy case: 이미지가 있지만 토큰이 없는 경우
                    dummy = (image_embeddings[0].sum() * 0.0).to(language_embeddings.dtype)
                    combined_embeddings = language_embeddings + dummy
